# -*- coding: utf-8 -*-
"""
Created on Mon Sep 18 21:29:57 2023
...
model = RELM_HiddenLayer(X_train,75)
model.classifisor_train(Y_train)
#y_score = model.predict_train(X_train)

y_score = pd.read_csv("D:\桌面\Project of RELM\ROC曲线\y_score train.csv")
...
label_binarizer = LabelBinarizer().fit(y1_train)
y_onehot_test = label_binarizer.transform(y1_train)
...
plt.title("Train set One-vs-Rest multiclass ROC curves", fontsize=12,fontweight='heavy',family='Arial')
font = {'family': 'Arial','size':10,"weight":'heavy'}
plt.legend(prop=font, frameon=False)
plt.text(-0.1, 1.08, '(b)', fontsize=12,fontweight='heavy',family='Arial')
plt.show()